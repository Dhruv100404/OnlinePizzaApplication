...
namespace OnlinePizzaWebApplication.Repositories
{
    public class PizzaRepository : IPizzaRepository
    {
        ...
        public IEnumerable<Pizzas> Pizzas => _context.Pizzas.Include(p => p.Category).Include(p => p.Reviews); //include here
        ...
        {
            return await _context.Pizzas.Include(p => p.Category).Include(p => p.Reviews).ToListAsync();
        }

        public IEnumerable<Pizzas> GetAllIncluded()
        {
            return _context.Pizzas.Include(p => p.Category).Include(p => p.Reviews).ToList();
        }
        ...
        {
            return _context.Pizzas.Include(p => p.Category).Include(p => p.Reviews).FirstOrDefault(p => p.Id == id);
        }

        public async Task<Pizzas> GetByIdIncludedAsync(int? id)
        {
            return await _context.Pizzas.Include(p => p.Category).Include(p => p.Reviews).FirstOrDefaultAsync(p => p.Id == id);
        }
